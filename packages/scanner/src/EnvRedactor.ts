...
export const ENV_MASK = '<HIDDEN_BY_QUELL>';

export class EnvRedactor {
     ...
     *    a line we print.
     */
    public static redact(fileContent: string): string {
        ...
        for (const line of lines) {
            ...
            if (openQuote) {
                if (this.closesQuote(line, openQuote)) { openQuote = null; }
                continue;
            }
            ...
            if (!match || !this.looksLikeEnvKey(match[1])) {
                out += `# ${ENV_MASK} (unparsed line withheld)\n`;
                continue;
            }
     ...
     * returned unchanged.
     */
    private static maskCommentedAssignment(line: string): string {
        const match = /^(\s*#+\s*)((?:export\s+)?[A-Za-z_][A-Za-z0-9_.-]*)(\s*=\s*)(\S.*)$/.exec(line);
        if (!match) { return line; }

        const key = match[2].replace(/^export\s+/, '');
        if (!this.looksLikeEnvKey(key)) { return `# ${ENV_MASK} (unparsed comment withheld)`; }

        return `${match[1]}${match[2]}=${ENV_MASK}`;
    }
     ...
     */
    public static looksLikeEnvKey(key: string): boolean {
    ...
    private static closesQuote(s: string, quote: string): boolean {
