...
type HiddenFamily = 'tag' | 'bidi' | 'bidi-mark' | 'joiner' | 'variation' | 'invisible';
 ...
 */
function isBidiChar(cp: number): boolean {
    return (
 ...
 */
function isBidiMark(cp: number): boolean {
    return (
 ...
 */
function isJoinerScript(cp: number | undefined): boolean {
...
const PHRASE_PATTERNS: PhrasePattern[] = [
    ...
    // ── Data exfiltration directives ─────────
    {
        ...
        regex: new RegExp(
            NEGATED +
            String.raw`\b(?:send|post|upload|transmit|exfiltrate|leak|forward|email|publish|share)\s+(?:me\s+|us\s+|the\s+|all\s+|your\s+|any\s+)*(?:contents?\s+of\s+)?(?:\.env\b|env(?:ironment)?\s+(?:vars?|variables?)|credentials?|secrets?|api[\s_-]*keys?|access[\s_-]*tokens?|private[\s_-]*keys?|ssh[\s_-]*keys?|password)`,
            'i',
        ),
...
export class PromptGuard {
    ...
    public static get phrasePatternCount(): number {
     ...
     */
    public static scan(text: string, config: GuardConfig = DEFAULT_GUARD_CONFIG): GuardResult {
        ...
        const isWhitelisted = (value: string): boolean => whitelist.some((re) => re.test(value));
        ...
        const push = (f: InjectionFinding): boolean => {
            if (isWhitelisted(f.raw)) { return true; }
            ...
            if (SEVERITY_RANK[f.severity] <= minRankHeld) { return true; }
            ...
            for (let i = 1; i < findings.length; i++) {
                ...
                if (r < minRankHeld) { minRankHeld = r; }
            }
     ...
     */
    public static strip(text: string): { text: string; removed: number } {
        let out = '';
        let removed = 0;
        let i = 0;
     ...
     */
    public static decodeTagPayload(raw: string): string {
        let out = '';
        for (const ch of raw) {
     ...
     */
    private static hiddenFamilyAt(text: string, index: number, cp: number): HiddenFamily | null {
        if (isTagChar(cp)) { return 'tag'; }
        if (isBidiChar(cp)) { return 'bidi'; }
        if (isBidiMark(cp)) { return 'bidi-mark'; }
        ...
        if (cp === 0x200c || cp === 0x200d) {
            ...
            if (cp === 0x200d && isEmojiBase(prev) && isEmojiBase(next)) { return null; }

            // Between two characters of a script that requires joiners, this is
            // spelling, not smuggling.
            if (isJoinerScript(prev) && isJoinerScript(next)) { return 'joiner'; }
    ...
    private static scanHiddenCharacters(text: string, push: (f: InjectionFinding) => boolean): void {
        let i = 0;
        ...
        while (i < text.length) {
            ...
            while (j < text.length) {
                ...
                if (this.hiddenFamilyAt(text, j, c) !== family) { break; }
                j += s;
            }

            const raw = text.slice(start, j);
            if (!this.emitHiddenFinding(family, raw, start, push)) { return; }
            i = j;
        }
    }
    ...
    private static emitHiddenFinding(
        ...
        raw: string,
        index: number,
        push: (f: InjectionFinding) => boolean,
    ): boolean {
        ...
        switch (family) {
            case 'tag': {
                ...
                return push({
                    ...
                    raw,
                    detail:
                        `${count} character(s) from the Unicode Tags block (U+E0000–U+E007F). These render as nothing ` +
            ...
            case 'bidi':
                return push({
                    ...
                    raw,
                    detail:
                        `${count} bidirectional override/isolate character(s). These open a scope that reorders how text ` +
            ...
            case 'joiner':
                return push({
                    ...
                    raw,
                    detail:
                        `${count} zero-width joiner/non-joiner character(s) between characters of a script that requires ` +
            ...
            case 'bidi-mark':
                return push({
                    ...
                    raw,
                    detail:
                        `${count} directional mark(s) (LRM/RLM/ALM). These are normal in Arabic and Hebrew text, where ` +
            ...
            case 'variation':
                return push({
                    ...
                    raw,
                    detail:
                        `${count} variation selector(s) not attached to an emoji. Each one carries a byte, so a run of ` +
            ...
            default:
                return push({
                    ...
                    raw,
                    detail:
                        `${count} zero-width or invisible character(s). ` +
                        (count >= 8
    ...
    private static scanPhrases(text: string, push: (f: InjectionFinding) => boolean): void {
    ...
    private static scanHomoglyphs(text: string, push: (f: InjectionFinding) => boolean): void {
        ...
        while ((match = WORD.exec(text)) !== null) {
            ...
            if (!hasLatin || (!cyrillic && !greek)) { continue; }
            ...
            const ok = push({
                ...
                raw: word,
                detail:
                    `"${word}" mixes Latin with ${script} characters that look identical to Latin ones. ` +
