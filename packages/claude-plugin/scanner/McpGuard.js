...
exports.McpGuard = exports.DEFAULT_MCP_CONFIG = void 0;
const SecretScanner_1 = require("./SecretScanner");
const PromptGuard_1 = require("./PromptGuard");
exports.DEFAULT_MCP_CONFIG = {
    scanner: SecretScanner_1.DEFAULT_CONFIG,
    guard: PromptGuard_1.DEFAULT_GUARD_CONFIG,
...
class McpGuard {
     ...
     */
    static isMcpConfigPath(filePath) {
        ...
        if (base === 'mcp_config.json' && (parent === '.windsurf' || parent === '.codeium')) {
            return true;
        }
        return false;
    }
     ...
     */
    static scanConfig(jsonText, config = exports.DEFAULT_MCP_CONFIG) {
        const findings = [];
        ...
        if (state.truncated) {
            findings.push({
                ...
                severity: 'medium',
                detail: 'This config is deep or large enough that Quell stopped walking it before the end, so parts were ' +
    ...
    static scanCredentialBlocks(serverName, server, config, findings) {
        for (const blockName of SCANNED_BLOCKS) {
            ...
            for (const [key, value] of Object.entries(block)) {
                ...
                if (WHOLE_VALUE_INDIRECTION.test(text)) {
                    continue;
                }
                ...
                findings.push({
                    ...
                    serverName,
                    key: `${blockName}.${key}`,
                    detail: `A literal ${types.join(' / ')} is stored in this server's \`${blockName}\` block. ` +
     ...
     */
    static scanInvocation(serverName, server, config, findings) {
        const parts = [];
        ...
        if (Array.isArray(server.args)) {
            for (const arg of server.args) {
                ...
                if (text !== null && !WHOLE_VALUE_INDIRECTION.test(text)) {
                    parts.push(text);
                }
            }
        }
        ...
        findings.push({
            ...
            serverName,
            key: 'args',
            detail: `A literal ${types.join(' / ')} is passed on this server's command line. Arguments show up in the ` +
                `process list, so every other process on the machine can read them, not just anyone opening the file.`,
        });
    }
    // ── Transport ────────────────────────────
    static scanTransport(serverName, server, findings) {
        ...
        if (/^http:\/\//i.test(url.trim()) && !local) {
            findings.push({
                ...
                serverName,
                key: 'url',
                detail: 'This server is reached over plain HTTP at a non-local host. Everything sent to it — file ' +
        ...
        if (!local) {
            findings.push({
                ...
                serverName,
                key: 'url',
                detail: 'This server runs remotely, so its operator sees whatever your assistant sends it, and its tool ' +
                    'definitions can change after you approved them. Worth knowing; not a defect in itself.',
            });
        }
    }
    // ── Tool descriptions (poisoning) ────────
    static scanDescriptions(serverName, server, config, findings, state, maxSites, maxDepth) {
        const seen = new Set();
        const walk = (node, depth) => {
            if (state.sites >= maxSites) {
            ...
            for (const [key, value] of Object.entries(node)) {
                if (typeof value === 'string' && MODEL_FACING_KEY.test(key)) {
                    ...
                    state.sites++;
                    const { findings: injected } = PromptGuard_1.PromptGuard.scan(value, config.guard);
                    for (const f of injected) {
                        ...
                        findings.push({
                            ...
                            serverName,
                            key,
                            detail: `A \`${key}\` field on this server contains ${f.type.toLowerCase()}. Tool descriptions ` +
     ...
     */
    static detectSecretTypes(text, scannerConfig) {
        try {
            const { secrets, detectedTypes } = SecretScanner_1.SecretScanner.redact(text, scannerConfig);
 ...
 */
function coerceScannable(value) {
    ...
    if (Array.isArray(value)) {
        const parts = value.map(coerceScannable).filter((s) => s !== null);
...
//# sourceMappingURL=McpGuard.js.map