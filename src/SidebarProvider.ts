...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(data => {
            switch (data.type) {
                case 'action':
                    // Security: Validate command to prevent arbitrary command execution via webview XSS
                    if (typeof data.command === 'string' && data.command.startsWith('quell.')) {
                        ...
                        console.warn(`Blocked attempt to execute unauthorized command from webview: ${data.command}`);
                    }
                    break;
            }
        });
