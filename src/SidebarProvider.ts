...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(data => {
            switch (data.type) {
                case 'action':
                    // Security: Strictly validate command prefixes to prevent arbitrary command execution from potential XSS
                    if (typeof data.command === 'string' && data.command.startsWith('quell.')) {
                        ...
                        console.warn(`[Security] Blocked execution of unapproved command from webview: ${data.command}`);
                    }
                    break;
            }
        });
